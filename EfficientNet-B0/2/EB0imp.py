    ...
    def __getitem__(self, idx):
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                ...
                v_cell = v_channel[i*cell_size:(i+1)*cell_size, j*cell_size:(j+1)*cell_size]
                
                valid_mask = v_cell > 0.05
                
                if np.sum(valid_mask) > 0:
                    h_mean, h_std = np.mean(h_cell[valid_mask]), np.std(h_cell[valid_mask])
                    s_mean, s_std = np.mean(s_cell[valid_mask]), np.std(s_cell[valid_mask])
                else:
                    h_mean, h_std, s_mean, s_std = 0.0, 0.0, 0.0, 0.0
                    
                color_features.extend([h_mean, h_std, s_mean, s_std])
...
class GridColorFusionMaizeNet(nn.Module):
    def __init__(self, num_classes=3, dropout_rate=0.4):
        ...
        cnn_out_features = base_net.classifier[1].in_features
    ...
    def forward(self, x_img, x_color):
        ...
        c = self.color_mlp(x_color) 
        
        fused = torch.cat([x, c], dim=1) 
...
if __name__ == '__main__':
    ...
    print(f"Environment: {CONFIG['device']} | AUG_MODE: [{AUG_MODE.upper()}] | Dataset: [Cleaned Data with Grid Color]")
    ...
    backbone_params = [p for n, p in model.named_parameters() if 'cnn_features' in n]
    custom_params = [p for n, p in model.named_parameters() if 'cnn_features' not in n]
    
    optimizer = optim.AdamW([
        {'params': backbone_params, 'lr': CONFIG['learning_rate']},
        {'params': custom_params, 'lr': CONFIG['learning_rate'] * 5}
    ...
    best_val_acc = 0.0
    best_model_path = f'best_model_{AUG_MODE}_cleaned_gridcolor.pth'
    ...
    plot_training_curves(history, exp_name=f"{AUG_MODE}_cleaned_gridcolor")
    
    model.load_state_dict(torch.load(best_model_path, weights_only=True))
    evaluate_and_save_metrics(model, val_loader, CONFIG['device'], exp_name=f"{AUG_MODE}_cleaned_gridcolor")
